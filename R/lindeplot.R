##EXAMPLE AND SCRIPT TO TEST THE LINDEMAN PLOT FUNCTION##
#############################################################

#set working directory
library(enaR)

energetics = read.csv('../data/99_eul_energ.csv',header=TRUE,sep=';',dec=",",row.names=1)
flows = read.csv('../data/99_eul_flow.csv',header=TRUE,sep=';',dec=",",row.names=1)
Living <- c(rep(TRUE,13),FALSE,FALSE,FALSE)

#define primary producers and non-living
primprod <- c('Phytoplankton','Microphytobenthos','Zostera')
nonliving <- c('DOC','susPOC','sedPOC')

m <- pack(flow=flows,input=energetics$input,respiration=energetics$respiration,export=energetics$output,output=energetics$respiration+energetics$output,storage=energetics$biomass,living=Living)
enatroagg <- enaTroAgg(m)

###Lindeman plot function##  ##NOTE Style of the plot according to Baird et al., 2004, 2007###

plot.lindeman <- function(enatroagg){
