...
plot.lindeman <- function(x, primprod, balance.override=FALSE){
...
if(comp==1){text(x2c+2.6,y2c+2.2,labels=round(sum(energetics[c(primprod,nonliving),'output'],na.rm=T)
###note: necessary to know where the function can find output values belonging to primprod uand non-living
,2),cex=0.95,pos=2)
