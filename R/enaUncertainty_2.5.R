enaUncertainty=function(x = 'network object', p.err=NA, Fu.sd=NA, zu.sd=NA, yu.sd=NA, Fu.bot=NA, zu.bot=NA, yu.bot=NA, Fu.top=NA, zu.top=NA, yu.top=NA, iter=100){
	
   if (class(x) != 'network'){warning('x is not a network class object')}					   # check object class
   
   #if (p.err == 0){warning('Zero error given, using 0.0001% error')}					   # zero error warning
   
   if (is.na(Fu.sd)[1] == FALSE && is.na(zu.sd)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.sd)[1] == FALSE && is.na(yu.sd)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.bot)[1] == FALSE && is.na(zu.bot)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.bot)[1] == FALSE && is.na(yu.bot)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.top)[1] == FALSE && is.na(zu.top)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.top)[1] == FALSE && is.na(yu.top)[1] == TRUE){warning('Insufficient uncertainty data')}					   # check uncertainty data inputs
   if (is.na(Fu.top)[1] == FALSE && is.na(Fu.bot)[1] == TRUE){warning('Lower limit uncertainty data not detected')}					   # check uncertainty data inputs
   if (is.na(Fu.bot)[1] == FALSE && is.na(Fu.top)[1] == TRUE){warning('Upper limit uncertainty data not detected')}					   # check uncertainty data inputs
	...
	upper.percent = 1+(p.err/100)
	}else if(p.err == 0){             # this is here because xsample breaks with zero percent error, maybe an error message would be better?
	...
	for(h in (length(U$z)+length(U$y)+1):(length(U$z)+length(U$y)+nrow(fluxes))){
		lower.percent[h] = Fu.bot[fluxes[(h-length(U$z)-length(U$y)),1],fluxes[(h-length(U$z)-length(U$y)),2]]/
								U$F[fluxes[(h-length(U$z)-length(U$y)),1],fluxes[(h-length(U$z)-length(U$y)),2]]
		}
	...
	for(h in (length(U$z)+length(U$y)+1):(length(U$z)+length(U$y)+nrow(fluxes))){
		upper.percent[h] = Fu.top[fluxes[(h-length(U$z)-length(U$y)),1],fluxes[(h-length(U$z)-length(U$y)),2]]/
								U$F[fluxes[(h-length(U$z)-length(U$y)),1],fluxes[(h-length(U$z)-length(U$y)),2]]
		}
	...
	print('ready for xSample') #take this out later, just to see where function is getting stuck 
