...
### UA with standard deviations

Fu.sd = matrix(c(0,10.3,0,2.2,0,
             3.1,0,0,0,1.8,
             2.3,0,0,0,0,
             5.1,0.2,0,0,0,
...
zu.sd = c(3.1,3.2,0,0,0)
yu.sd = c(0,0,4.7,0,4.1)

### UA with individual input for lower and upper limits of each value

Fu.bot = matrix(c(0,5,0,2.2,0,
             0.9,0,0,0,1.8,
             3,0,0,0,0,
             5.1,1,0,0,0,
             0,0,0.3,2.9,0), 5, 5, byrow = TRUE)
Fu.bot=t(Fu.bot)

zu.bot = c(3.1,3.2,0,0,0)
yu.bot = c(0,0,1.1,0,8.1)

Fu.top = matrix(c(0,12.3,0,6.1,0,
             3.9,0,0,0,5.8,
             100,0,0,0,0,
             18,10.1,0,0,0,
             0,0,1.9,15.1,0), 5, 5, byrow = TRUE)
Fu.top=t(Fu.top)

zu.top = c(13.1,8.2,0,0,0)
yu.top = c(0,0,4.7,0,14.1)

############################################################
############### Apply uncertainty analysis #################
############################################################

source('enaUncertainty_2.5.R')

# vary all fluxes by +/- a percent
     #PM = enaUncertainty(M, p.err=10, iter=100)

# vary each flux by its standard deviation
     #PM = enaUncertainty(M, Fu.sd=Fu.sd, zu.sd=zu.sd, yu.sd=yu.sd, iter=100)

# give lower and upper limits for each flux
     PM = enaUncertainty(M, Fu.bot=Fu.bot, zu.bot=zu.bot, yu.bot=yu.bot, Fu.top=Fu.top, zu.top=zu.top, yu.top=yu.top, iter=100)
