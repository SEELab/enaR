...
<<eva=false,echo=false,results=hide>>=
  rm(list=ls())
par(mfrow=c(1,1))
options(width=72)
...
It determines the pairwise control relationships between the nodes 
in the network. Recent updates to control analysis 
\citep[e.g.,][]{schramski06, schramski07} have also been included 
in this function. Table~\ref{tab:control} summarizes the function output.
...
provides results similar to NETWRK's 'Full Cycle Analysis'. 
 Cycles in a network are grouped together into disjoint nexuses and each 
 nexus is characterized by a weak arc. This function gives details of the 
 individual cycles along with the disjoint nexuses present in the network. 
 Table~\ref{tab:cycle} summarizes the function output.
...
%It also gives the distribution of flow into cycles of different lengths and                  %% Determine whether necessary ~Pawan
 ...
 \begin{table*}[t]
  \caption{Data frames, matrices and graph-level network statistics returned by the \textit{enaR}    %%Data frames may be called tables ~Pawan
    \texttt{enaCycle} function, which are based on \citep{ulanowicz83}.}\label{tab:cycle}
  \center
  \begin{small}
    \begin{tabular}{l l}
      \textbf{Label} & \textbf{Description} \\ \hline \\[-1.5ex]
      \multicolumn{2}{l}{\textit{Data frames}} \\[1ex]
      Table.cycle & data frame of all cycles in the network\\
      Table.nexus & data frame with details of the disjoint nexuses present in the network\\[1ex]
      \multicolumn{2}{l}{\textit{Matrices}} \\[1ex]
      CycleDist$_{n \times 1}$ & vector of flows cycling in loops of different sizes \\  % these have names in the literature
...
The Trophic Aggregation algorithm unleashes the Trophic Structure 
...
It is crucial for this algorithm that the cycles among the living nodes of the 
 ...
 \begin{table*}[t]
  ...
  \begin{small}
    \begin{tabular}{l l}
      ...
      \multicolumn{2}{l}{\textit{Network Statistics}} \\[1ex]
      Detritivory & flow from the detrital pool to the second trophic level \\
