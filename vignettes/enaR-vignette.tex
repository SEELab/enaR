...
%%Tasks
%1. Section 1.3 re-write to incorporate new mode of accessing attributes
%2. 
...
\Abstract{ Ecosystem Network Analysis (ENA) provides a framework for
  ...
  systems. Here, we present the \textit{enaR} package, which provides
  a broad representation of many of the core tools developed by the
  ENA community. We provide a detailed description of how to use the
  primary functions of the package for the analysis of single models
  and simultaneous, synthetic analysis of multiple ecosystem models.}

\Keywords{ecology, ENA, ecosystems, species interactions, networks,
  \proglang{R}} \Plainkeywords{ecology, ENA, ecosystems, species
  interactions, networks, R} %% without formatting
...
\cite{borrett12_netecol}.  These systems are modeled as
thermodynamically conserved energy--matter exchanges between species,
groups of species, or non-living componets (e.g., dead organic matter)
of the ecosystem, and the weighted, directed edges are the quantified
transfers of energy or matter. Constructing these models is no small
task \cite{fathCITE}; however, the system level analysis that ENA
provides can reveal often surprising unsights into the behavior of
complex systems and from these applications a new theoretical
understanding of ecosystems has emerged \citep{higashi91, belgrano05,
  jorgensen07_newecology}.  For example, ENA has been used to show the
importance of indirect effects in ecosystems \citep{patten83,
  higashi89, salas11_did}.  Recently, scientists have been applying
these methods to understand trophic dynamics
...
% objectives
Multiple, independent software packages have been created to support
...
inhibited the development and further implementation of theory.
...
The package is written in the \textbf{R} language, which is free and
open-source.  Due largely to this, \textbf{R} is now one of the most widely
...
in \textbf{R} that contains the model data for subsequent analysis.  To
...
flow matrix.  The \texttt{read.wand} function will create an \textbf{R}
...
We can use the excellent graphics capabilities of \textbf{R} to make fancier
...
One advantage of the \textit{enaR} \textbf{R} package is that it simplifies this
...
Another advantage of building the \textit{enaR} package in \textbf{R} is that
...
There are other \textbf{R} packages that have graph and network analysis tools,
...
their data with other \textbf{R} packages for graph and network analysis like
\textit{sna} and \textit{iGraph}.  In summary, we hope you find this
package useful for your ENA needs.

\section{Acknowledgements}

We would like to thank David Hines and Pawandeep Singh for
contributions to the code and reviewing the vignette.  We also thank
Ursula Scharler, Shaoqing Chen, Emily Oxe, and John Mejaski for
testing early versions of the package and provided helpful feedback.
